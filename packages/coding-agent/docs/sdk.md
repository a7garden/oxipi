...
```bash
npm install @oxipi/coding-agent
```
...
```typescript
import { createAgentSession } from "@oxipi/coding-agent";
...
The `Agent` class (from `@oxipi/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.oxipi/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.oxipi/extensions/`)
- Project skills:
  - `.oxipi/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.oxipi/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.oxipi/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@oxipi/coding-agent";
...
// Default: uses ~/.oxipi/agent/auth.json and ~/.oxipi/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@oxipi/coding-agent";
...
import { createAgentSession, defineTool } from "@oxipi/coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.oxipi/agent/extensions/`, `.oxipi/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@oxipi/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@oxipi/coding-agent";
...
1. Global: `~/.oxipi/agent/settings.json`
2. Project: `<cwd>/.oxipi/settings.json`
