...
| `.oxipi/settings.json` | Project (current directory) |
...
Paths in `~/.oxipi/agent/settings.json` resolve relative to `~/.oxipi/agent`. Paths in `.oxipi/settings.json` resolve relative to `.pi`. Absolute paths and `~` are supported.
...
Project settings (`.oxipi/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.oxipi/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .oxipi/settings.json (project)
{
