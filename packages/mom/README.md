...
# Option 2: use /login command in pi agent, then copy/link auth.json to ~/.oxipi/mom/
...
- run interactive coding agent session: `npx @oxipi/coding-agent`
- enter `/login` command
  - choose "Anthropic" provider
  - follow instructions in the browser
- link `auth.json` to mom: `ln -s ~/.oxipi/agent/auth.json ~/.oxipi/mom/auth.json`
...
You can find a set of basic skills at [github.com/a7garden/oxipi-skills](https://github.com/a7garden/oxipi-skills). Just tell mom to clone this repository into `/workspace/skills/oxipi-skills` and she'll help you set up the rest.
...
Update mom anytime with `npm install -g @oxipi/mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
