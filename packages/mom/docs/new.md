...
```typescript
import { type AgentSessionEvent } from "@oxipi/coding-agent";
...
import type { MomCustomTool, ToolAPI } from "@oxipi/mom";
...
2. `~/.oxipi/mom/tools/**/index.ts` (global, recursive)
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@oxipi/ai` and `@oxipi/mom` are aliased to the running mom bundle.
