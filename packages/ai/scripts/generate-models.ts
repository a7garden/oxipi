...
`;
	...
	// Write per-provider split files for better IDE performance and merge conflict reduction
	const srcDir = join(packageRoot, "src", "providers", "models-generated");
	writeFileSync(join(srcDir, "index.ts"), `// Auto-generated by scripts/generate-models.ts\n// Do not edit manually\n\n${sortedProviderIds.map(p => `export * from "./${p}.js";`).join("\n")}\n`);
	console.log(`Generated src/providers/models-generated/index.ts`);

	for (const providerId of sortedProviderIds) {
		const models = providers[providerId];
		let providerOutput = `// Auto-generated by scripts/generate-models.ts\n// Do not edit manually - run 'npm run generate-models' to update\n\n`;
		providerOutput += `import type { Model } from "../../types.js";\n\n`;
		providerOutput += `export const ${providerId.replace(/-/g, "_")}_MODELS = {\n`;

		const sortedModelIds = Object.keys(models).sort();
		for (const modelId of sortedModelIds) {
			const model = models[modelId];
			providerOutput += `\t"${model.id}": {\n`;
			providerOutput += `\t\tid: "${model.id}",\n`;
			providerOutput += `\t\tname: "${model.name}",\n`;
			providerOutput += `\t\tapi: "${model.api}",\n`;
			providerOutput += `\t\tprovider: "${model.provider}",\n`;
			if (model.baseUrl !== undefined) {
				providerOutput += `\t\tbaseUrl: "${model.baseUrl}",\n`;
			}
			if (model.headers) {
				providerOutput += `\t\theaders: ${JSON.stringify(model.headers)},\n`;
			}
			if (model.compat) {
				providerOutput += `\t\tcompat: ${JSON.stringify(model.compat)},\n`;
			}
			providerOutput += `\t\treasoning: ${model.reasoning},\n`;
			providerOutput += `\t\tinput: [${model.input.map(i => `"${i}"`).join(", ")}],\n`;
			providerOutput += `\t\tcost: {\n`;
			providerOutput += `\t\t\tinput: ${model.cost.input},\n`;
			providerOutput += `\t\t\toutput: ${model.cost.output},\n`;
			providerOutput += `\t\t\tcacheRead: ${model.cost.cacheRead},\n`;
			providerOutput += `\t\t\tcacheWrite: ${model.cost.cacheWrite},\n`;
			providerOutput += `\t\t},\n`;
			providerOutput += `\t\tcontextWindow: ${model.contextWindow},\n`;
			providerOutput += `\t\tmaxTokens: ${model.maxTokens},\n`;
			providerOutput += `\t} satisfies Model<"${model.api}">,\n`;
		}

		providerOutput += `} as const;\n`;
		writeFileSync(join(srcDir, `${providerId}.ts`), providerOutput);
		console.log(`Generated src/providers/generated/${providerId}.ts (${Object.keys(models).length} models)`);
	}
